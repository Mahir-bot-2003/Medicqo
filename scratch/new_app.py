...
class App(ctk.CTk):
    ...
    def build_sidebar(self):
        self.sidebar = ctk.CTkFrame(self, fg_color=THEME_SIDEBAR, corner_radius=0, width=220)
    ...
    def build_header(self):
        ...
        ctk.CTkLabel(profile_frame, text=f"Dr. {self.current_doctor['name']}", font=("Helvetica", 14, "bold"), text_color=THEME_TEXT_MAIN).pack(side="top", anchor="e")
    ...
    def show_add_patient_view(self):
        ...
        dob_entry = ctk.CTkEntry(form, placeholder_text="DOB (YYYY-MM-DD)", width=400, height=40)
        dob_entry.pack(pady=10, padx=40)
    ...
    def show_prescription_view(self, patient):
        ...
        symp_box = ctk.CTkTextbox(left_col, height=80, fg_color="#f9fafb", border_color="#e5e7eb", border_width=1)
        ...
        diag_box = ctk.CTkTextbox(left_col, height=60, fg_color="#f9fafb", border_color="#e5e7eb", border_width=1)
        ...
        def add_med_row():
            ...
            n = ctk.CTkEntry(row, placeholder_text="Medicine Name", width=120, height=30)
            ...
            dur = ctk.CTkEntry(row, placeholder_text="Days", width=50, height=30)
        ...
        def submit():
            ...
            payload = {
                "symptoms": symp_box.get("1.0", "end").strip(),
                "diagnosis": diag_box.get("1.0", "end").strip(),
                "notes": notes_box.get("1.0", "end").strip(),
                "medicines": meds,
                "patient_id": patient["id"],
                "doctor_id": self.current_doctor["id"],
                "send_channel": send_channel.get()
            }
        ...
        ctk.CTkButton(actions, text="Generate & Send PDF", fg_color=THEME_PRIMARY, height=40, command=submit).pack(side="right")
